...
public class Persona extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("persona".equals(action)){
			...
			StringBuffer result = sendRequest(url);
			response.getWriter().write(result.toString());
		} else if("personaVirtual".equals(action)){
			Map<String, String> requestMap = CommonMethod.requestParameter(request);
			url = getServletConfig().getServletContext().getInitParameter("pythonwebservice")
					+"/VirtualPersona/"
					+"sex="+new String(Base64.encodeBase64String(CommonMethod.null2str(requestMap.get("sex")).getBytes()))
					+"&age="+new String(Base64.encodeBase64String(CommonMethod.null2str(requestMap.get("age")).getBytes()))
					+"&px3="+new String(Base64.encodeBase64String(CommonMethod.null2str(requestMap.get("px3")).getBytes()))
					+"&px4="+new String(Base64.encodeBase64String(CommonMethod.null2str(requestMap.get("px4")).getBytes()))
					+"&px5="+new String(Base64.encodeBase64String(CommonMethod.null2str(requestMap.get("px5")).getBytes()))
					+"&px6="+new String(Base64.encodeBase64String(CommonMethod.null2str(requestMap.get("px6")).getBytes()))
					+"&px7="+new String(Base64.encodeBase64String(CommonMethod.null2str(requestMap.get("px7")).getBytes()))
					+"&px8="+new String(Base64.encodeBase64String(CommonMethod.null2str(requestMap.get("px8")).getBytes()))
					+"&px9="+new String(Base64.encodeBase64String(CommonMethod.null2str(requestMap.get("px9")).getBytes()));
