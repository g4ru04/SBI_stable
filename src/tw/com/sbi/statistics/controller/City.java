...
public class City extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		logger.trace("From: "+this.getClass().getSimpleName());
		request.setCharacterEncoding("UTF-8");
		response.setCharacterEncoding("UTF-8");
		
		Map<String, String> mapRqstPara = SharedFunction.getFormatedRqstPara(request);//SharedFunction.configRqstPara(request);
		
		CityDAO dao = new CityDAO();
		dao.setRequest(mapRqstPara);
		
		String action = mapRqstPara.get("action");
		
		if(action!=null){
			dao.execAccess(action);
			response.getWriter().write(dao.getResponseJsonString());
		}else{
