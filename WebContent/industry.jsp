...
<script>
	...
	function show_chart(chartOptions, isChart) {
		...
		if(window.scenario_record){scenario_record("動態統計-目標產業","[ '"+$("#country").val()+"', '"+$("#industry_type").val()+"', '"+$("#source").val()+"', '"+$("#subsource").val()+"', ["+categories_list+"], ["+categories_year_list+"]]","selector","#view",function(){
			$.ajax({
				type : "POST",
				url : "industry.do",
				data : {
				...
				success : function(result) {
					...
					for (var row = 0; row < row_count; row++) {
						...
						for (var column = 0; column < column_count; column++) {
							...
							$.each(json_obj, function(i, item) {
								row_unit=item.unit;
								if (row_title_arr[row].toLowerCase()  == item.categories.toLowerCase()  && column_title_arr[column] == item.categoriesYear) {
									column_temp = '<td style="text-align:right;">' + item.categoriesData + '</td>';
								}
							});
