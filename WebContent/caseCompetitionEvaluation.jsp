	...
	$(function() {
		...
		$("#btn_step2_next").click(function(e) {
			...
			if (step2_index < step2_arr.length ) {
				...
				var titleSize = competition_name_arr.length + 1;
				titleSize = 100/titleSize;
				
				var tempDiv = "";
				var temp_text = "";
				
				for (var k = 0; k < competition_name_arr_count; k++) {
					temp_text += '<th style="width:'+titleSize+'%">' + competition_name_arr[k] + '</th>';
				}
		...
		function getCaseNotFinish() {
			$.ajax({
				...
				data : {
					action : "getCaseCompetitionNotFinish"
				},
				success : function(result) {
					var json_obj = $.parseJSON(result);
					...
					$.each(json_obj, function(i, item) {
						...
						competition_name_arr = item.competition_name.split(",");;
						competition_name_arr_count = competition_name_arr.length
						console.log("competition_name_arr:");
						console.log(competition_name_arr);
						...
						$('#competition_id').val(item.competition_id);
						console.log("Competition ID set :" + item.competition_id);
						console.log("Competition ID get :" + $('#competition_id').val());
						...
						console.log("bcircle_name: "+bcircle_name);
		...
		function confirm() {
			...
			for (var k = 0; k <competition_name_arr_count; k++) {
				...
				eval_ratio += competition_name_arr[k] + ',';
				for (var i = 1; i <= level1_count; i++) {
...
<input type="hidden" id="glb_privilege" value="<%= privilege %>" />
	...
	<div class="page-wrapper" >
		...
		<div id="msgAlert"></div>
	 	...
	 	<h2 id="title" class="page-title">競爭力決策評估</h2>
