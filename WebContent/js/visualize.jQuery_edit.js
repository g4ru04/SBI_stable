			...
			var tableData = {
				...
				yLabels: function(){
					...
					while( yLabels[yLabels.length-1] < topValue - loopInterval){
						...
						//Ben偷改要讓Label兩位數後面四捨五入
						//yLabels.push(yLabels[yLabels.length-1] + loopInterval);
						var to_deal_value = yLabels[yLabels.length-1] + loopInterval;
						var scale_ten = (topValue+"").length;
						var count=0
						Math.pow(0.1,scale_ten-2);
						if(scale_ten>2){
							to_deal_value = Math.round( to_deal_value * Math.pow(0.1,scale_ten-2) ) * Math.pow(10,scale_ten-2);
						}
						yLabels.push(to_deal_value);
					}
					//Ben因應上面的topValue修改
					//yLabels.push(topValue);
					yLabels.push(Math.round(topValue * 6 / 5));
			...
			bar: function(){
				...
				$.each(yLabels, function(i){  
					var thisLi = $('<li><span>'+this+'</span></li>')
						...
						var topOffset = label.height()/-2;
						//Ben偷改的 因為0的label不正
						//(原本設計大概因為預設是頁面完全剛好 所以為了讓最上下的標籤不被切掉 特別處理)
//						if(i == 0){ topOffset = -label.height(); }
//						else if(i== yLabels.length-1){ topOffset = 0; }
						label
							.css('margin-top', topOffset)
